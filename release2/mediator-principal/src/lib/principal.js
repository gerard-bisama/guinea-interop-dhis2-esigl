...
function setupApp() {
  ...
  app.get('/loadlogs', (req, res) => {
    ...
    url = url.toString();
    logger.log({
      level: levelType.info, operationType: typeOperation.startTheService, action: "/loadlogs", result: typeResult.iniate,
      message: `Lancement de de l'importation des logs dans Kibana`
    });
    customLibrairy.readAppLogFile(logFileName, (fileLogs) => {
      logger.log({
        level: levelType.info, operationType: typeOperation.normalProcess, action: "/loadlogs", result: typeResult.ongoing,
  ...
  app.get('/syncorgunit2fhir', (req, res) => {
    ...
    if (req.query.orgunitid) {
      ...
      logger.log({
        level: levelType.info, operationType: typeOperation.normalProcess, action: "/syncorgunit2fhir", result: typeResult.iniate,
    ...
    logger.log({
      level: levelType.info, operationType: typeOperation.getData, action: `/api/${orgUnitResource}.json`, result: typeResult.iniate,
    ...
    if (orgUnitId) {
      ...
      getListDHIS2OrgUnitByFilter(dhis2Token, filterExpresion, function (listOrgUnits) {
        ...
        if (listOrgUnits.length > 0) {
          ...
          logger.log({
            level: levelType.info, operationType: typeOperation.postData, action: "/fhir/Location", result: typeResult.iniate,
          ...
          saveBundle2Fhir(hapiToken, 'Location', bundle, function (hapiServerResponse) {
            ...
            if (hapiServerResponse.status == 200) {
              ...
              logger.log({
                level: levelType.info, operationType: typeOperation.postData, action: "/fhir/Location", result: typeResult.success,
                message: `${hapiServerResponse.message}`
              });
              logger.log({
                level: levelType.info, operationType: typeOperation.postData, action: "/syncorgunit2fhir", result: typeResult.success,
                message: `${hapiServerResponse.message}`
              });
              ...
              logger.log({
                level: levelType.error, operationType: typeOperation.postData, action: "/fhir/Location", result: typeResult.failed,
                message: `Erreur: ${hapiServerResponse.message}`
              });
              logger.log({
                level: levelType.error, operationType: typeOperation.postData, action: "/syncorgunit2fhir", result: typeResult.failed,
                message: `Erreur: ${hapiServerResponse.message}`
              });
          ...
          logger.log({
            level: levelType.warning, operationType: typeOperation.normalProcess, action: "/syncorgunit2fhir", result: typeResult.failed,
  ...
  app.get('/mapfacility2fhir', (req, res) => {
    ...
    logger.log({
      level: levelType.info, operationType: typeOperation.normalProcess, action: "/mapfacility2fhir", result: typeResult.iniate,
      message: `Lancement du processus de mapping des structures eSIGL=>DHIS2`
    });
    const hapiToken = `Basic ${btoa(config.hapiServer.username + ':' + config.hapiServer.password)}`;
    logger.log({
      level: levelType.info, operationType: typeOperation.getData, action: `${filePath}`, result: typeResult.iniate,
    ...
    customLibrairy.readeSIGLDataCSVFile(filePath, function (listStructures) {
      if (listStructures.length > 0) {
        logger.log({
          level: levelType.info, operationType: typeOperation.getData, action: `Lecture du fichier CSV des structures`, result: typeResult.success,
        ...
        logger.log({
          level: levelType.info, operationType: typeOperation.getData, action: `/fhir/Location`, result: typeResult.iniate,
        ...
        getListHAPILocationByIdChunk(hapiToken, listLocationIds, config.batchSizeFacilityFromHapi, function (listLocationToMap) {
          ...
          if (listLocationToMap.length > 0) {
            logger.log({
              level: levelType.info, operationType: typeOperation.getData, action: `/fhir/Location`, result: typeResult.success,
            ...
            if (updatedLocationBundle.entry && updatedLocationBundle.entry.length >= 1) {
              ...
              saveBundle2Fhir(hapiToken, fhirLocationResource, updatedLocationBundle, function (hapiServerResponse) {
                ...
                if (hapiServerResponse.status == 200) {
                  ...
                  logger.log({
                    level: levelType.info, operationType: typeOperation.postData, action: "/mapfacility2fhir", result: typeResult.success,
                  ...
                  logger.log({
                    level: levelType.info, operationType: typeOperation.postData, action: "/mapfacility2fhir", result: typeResult.failed,
                    message: `Erreur: ${hapiServerResponse.message}`
                  });
                  let responseMessage = `Erreur: ${hapiServerResponse.message}`;
                  let bodyMessage = `${updatedLocationBundle.entry.length} Location a charger dans HAPI FHIR`;
              ...
              logger.log({
                level: levelType.info, operationType: typeOperation.getData, action: "/mapfacility2fhir", result: typeResult.failed,
                message: `Erreur: Impossible de construire le Bundle de correspondance`
              });
        ...
        logger.log({
          level: levelType.error, operationType: typeOperation.getData, action: `${filePath}`, result: typeResult.failed,
          message: `Erreur: Aucune correspondance pour le mapping des structures`
        });
        logger.log({
          level: levelType.info, operationType: typeOperation.getData, action: "/mapfacility2fhir", result: typeResult.failed,
  ...
  app.get('/syncprogramproduct2fhir', (req, res) => {
    logger.log({
      level: levelType.info, operationType: typeOperation.normalProcess, action: "/syncprogramproduct2fhir", result: typeResult.iniate,
    ...
    logger.log({
      level: levelType.info, operationType: typeOperation.getData, action: `/sigl/program-products`, result: typeResult.iniate,
    ...
    async.eachSeries(orchestrations, (orchestration, nextOrchestration) => {
      ...
      localNeedle.get(orchUrl, options, function (err, resp) {
        if (err) {
          logger.log({
            level: levelType.error, operationType: typeOperation.getData, action: `/sigl/${orchestration.name}`, result: typeResult.failed,
        ...
        if (resp.statusCode && (resp.statusCode == 200)) {
          ...
          if (orchestration.name == "programs") {
            ...
            logger.log({
              level: levelType.info, operationType: typeOperation.getData, action: `/sigl/${orchestration.name}`, result: typeResult.ongoing,
      ...
      if (err) {
        logger.log({
          level: levelType.error, operationType: typeOperation.getData, action: `/sigl/program-products`, result: typeResult.failed,
          message: `SIGL: ${err.message}`
        });
      }
      logger.log({
        level: levelType.info, operationType: typeOperation.getData, action: `/sigl/program-products`, result: typeResult.success,
      ...
      let productProgram = customLibrairy.buildProgamProductRessourceBundle(programsList, productsList, programproductsList, config.esiglServer.url,
        config.extensionBaseUrlProductDetails, config.extensionBaseUrlProgramDetails, config.codeSystemIdentifiersType, config.codeSystemEntitiesType);
      //return res.send(productProgram[0]);
      if (productProgram && productProgram.length > 0) {
        let bundleProducts = productProgram[0];
        let bundlePrograms = productProgram[1];
        logger.log({
          level: levelType.info, operationType: typeOperation.postData, action: `/fhir/program-products`, result: typeResult.iniate,
        ...
        saveBundle2Fhir(hapiToken, productResource, bundleProducts, (hapiServerResponse) => {
          if (hapiServerResponse.status == 200) {
            logger.log({
              level: levelType.info, operationType: typeOperation.postData, action: `/fhir/product`, result: typeResult.success,
              message: `${hapiServerResponse.message}: ${bundleProducts.entry.length} produits mise a jour`
            });
            logger.log({
              level: levelType.info, operationType: typeOperation.postData, action: `/fhir/program`, result: typeResult.iniate,
            ...
            saveBundle2Fhir(hapiToken, programResource, bundlePrograms, (hapiServerResponse1) => {
              ...
              if (hapiServerResponse1.status == 200) {
                logger.log({
                  level: levelType.info, operationType: typeOperation.postData, action: `/fhir/program`, result: typeResult.success,
                  message: `${hapiServerResponse1.message}: ${bundlePrograms.entry.length} programmes mise a jour`
                });
                logger.log({
                  level: levelType.info, operationType: typeOperation.getData, action: `/syncprogramproduct2fhir`, result: typeResult.success,
                ...
                logger.log({
                  level: levelType.error, operationType: typeOperation.postData, action: `/fhir/program`, result: typeResult.failed,
                  message: `Erreur lors de la mise a jour des programmes: ${hapiServerResponse1.message}`
                });
                logger.log({
                  level: levelType.info, operationType: typeOperation.getData, action: `/syncprogramproduct2fhir`, result: typeResult.failed,
          ...
          else {//productResource
            logger.log({
              level: levelType.error, operationType: typeOperation.postData, action: `/fhir/product`, result: typeResult.failed,
              message: `Erreur lors de la mise a jour des produits: ${hapiServerResponse.message}`
            });
            logger.log({
              level: levelType.error, operationType: typeOperation.postData, action: `/syncprogramproduct2fhir`, result: typeResult.failed,
              message: `Erreur lors de la mise a jour des produits: ${hapiServerResponse.message}`
            });
            let responseMessage = `${bundleProducts.entry.length} produits et ${bundlePrograms.entry.length} programmes misent a jour`;
            let bodyMessage = `Lancement de la mise a jour des produits et programmes dans HAPI`;
        ...
        logger.log({
          level: levelType.error, operationType: typeOperation.normalProcess, action: `buildProgamProductRessourceBundle`, result: typeResult.failed,
          message: `Erreur lors de la transformation en Bundle des produits et programmes ESIGL`
        });
        logger.log({
          level: levelType.error, operationType: typeOperation.normalProcess, action: `/syncprogramproduct2fhir`, result: typeResult.failed,
          message: `Erreur lors de la transformation en Bundle des produits et programmes ESIGL`
        });
  ...
  app.get('/syncprogramproduct2dhis', (req, res) => {
    ...
    if (!programCode) {
      logger.log({
        level: levelType.info, operationType: typeOperation.normalProcess, action: "/syncprogramproduct2dhis", result: typeResult.iniate,
      ...
      let bodyMessage = `La variable programCode non definie`;
    ...
    logger.log({
      level: levelType.info, operationType: typeOperation.normalProcess, action: "/syncprogramproduct2dhis2", result: typeResult.iniate,
      message: `Lancement du processus de synchronisation des programmes-produits dans DHIS2`
    });
    const dhis2Token = `Basic ${btoa(config.dhis2Server.username + ':' + config.dhis2Server.password)}`;
    const hapiToken = `Basic ${btoa(config.hapiServer.username + ':' + config.hapiServer.password)}`;
    logger.log({
      level: levelType.info, operationType: typeOperation.getData, action: `/fhir/program`, result: typeResult.iniate,
    ...
    getListHapiResourceByFilter(hapiToken, fhirProgramResource, filterExpresion, (listProgramsEntries) => {
      ...
      logger.log({
        level: levelType.info, operationType: typeOperation.getData, action: `/fhir/program`, result: typeResult.success,
      ...
      async.eachSeries(listProgramsEntries, function (programEntry, nextStep) {
        ...
        logger.log({
          level: levelType.info, operationType: typeOperation.getData, action: "/fhir/product", result: typeResult.iniate,
        ...
        getListHAPIResourcesByIdsChunk(hapiToken, fhirProductResource, listResourceIds, config.batchSizeFacilityFromHapi, (listProducts) => {
          logger.log({
            level: levelType.info, operationType: typeOperation.getData, action: "/fhir/product", result: typeResult.success,
          ...
          logger.log({
            level: levelType.info, operationType: typeOperation.postData, action: `/api/${dhisCategoryOption}`, result: typeResult.iniate,
          ...
          saveMetadataList2Dhis(dhis2Token, dhisCategoryOption, listCategoryOptions, (dhisOpResponse) => {
            ...
            logger.log({
              level: levelType.info, operationType: typeOperation.postData, action: `/api/${dhisCategoryOption}`, result: typeResult.success,
            ...
            if (listProducts2Update.length > 0) {
              logger.log({
                level: levelType.info, operationType: typeOperation.getData, action: `/api/${dhisCategoryOption}`, result: typeResult.iniate,
              ...
              updateMetadataList2Dhis(dhis2Token, dhisCategoryOption, listProducts2Update, (dhisUpdateOperation) => {
                ...
                saveMetadataList2Dhis(dhis2Token, dhisCategory, programMetadataList, (dhisProgAddResponse) => {
                  ...
                  if (dhisProgAddResponse[0].httpStatus == "OK" || dhisProgAddResponse[0].httpStatus == "Created" || dhisProgAddResponse[0].httpStatus == "Conflict") {
                    ...
                    logger.log({
                      level: levelType.info, operationType: typeOperation.postData, action: `/api/${dhisCategoryOption}/${dhisIdProgram}`, result: typeResult.iniate,
                    ...
                    saveMetadataCollection2Dhis(dhis2Token, dhisCategory, dhisIdProgram, dhisCategoryOption, categoryOptionsCollection,
                      (dhisCreateCollectionOp) => {
                        ...
                        if (dhisCreateCollectionOp[0].httpStatus == "OK") {
                          ...
                          logger.log({
                            level: levelType.info, operationType: typeOperation.postData, action: `/api/${dhisCategoryOption}/${dhisIdProgram}`, result: typeResult.success,
                            message: `Ajout de la collection des produits dans la category ${programEntry.resource.name} dans DHIS2`
                          });
                          let programCatCombosMatadataList = [customLibrairy.buildCategoryCombosMetadata(programEntry.resource)];
                          logger.log({
                            level: levelType.info, operationType: typeOperation.postData, action: `/api/${dhisCategoryCombo}`, result: typeResult.iniate,
                          ...
                          saveMetadataList2Dhis(dhis2Token, dhisCategoryCombo, programCatCombosMatadataList, (dhisProgComboAddResponse) => {
                            if (dhisProgComboAddResponse[0].httpStatus == "OK" ||
                              ...
                              logger.log({
                                level: levelType.info, operationType: typeOperation.postData, action: `/api/${dhisCategoryCombo}`, result: typeResult.success,
                                message: `Creation de la ${dhisCategoryCombo} ${programEntry.resource.name} dans DHIS2`
                              });
                              ...
                              logger.log({
                                level: levelType.error, operationType: typeOperation.postData, action: `/api/${dhisCategoryCombo}`, result: typeResult.failed,
                          ...
                          operationOutcome = operationOutcome && false;
                          logger.log({
                            level: levelType.error, operationType: typeOperation.postData, action: `/api/${dhisCategory}`, result: typeResult.failed,
                    ...
                    operationOutcome = operationOutcome && false;
                    logger.log({
                      level: levelType.error, operationType: typeOperation.postData, action: "/saveMetadataList2Dhis", result: typeResult.failed,
                  ...
                  saveMetadataCollection2Dhis(dhis2Token, dhisCategory, dhisIdProgram, dhisCategoryOption, categoryOptionsCollection,
                    (dhisCreateCollectionOp) => {
                      if (dhisCreateCollectionOp[0].httpStatus == "OK") {
                        ...
                        operationOutcome = operationOutcome && false;
                        logger.log({
                          level: levelType.error, operationType: typeOperation.postData, action: `/${dhisCategory}`, result: typeResult.failed,
                          message: `Erreur:${dhisCreateCollectionOp[0].httpStatus}: echec de la creation de ${dhisCategory} ${programEntry.resource.name} dans DHIS2`
                        });
        ...
        if (operationOutcome) {
          logger.log({
            level: levelType.info, operationType: typeOperation.getData, action: `/syncprogramproduct2dhis`, result: typeResult.success,
          ...
          logger.log({
            level: levelType.info, operationType: typeOperation.getData, action: `/syncprogramproduct2dhis`, result: typeResult.failed,
  ...
  app.get('/updatecatcombodhis', (req, res) => {
    ...
    if (!programCode) {
      logger.log({
        level: levelType.info, operationType: typeOperation.normalProcess, action: "/updatecatcombodhis", result: typeResult.iniate,
        message: `Le parametre programCode est obligatoire`
      });
      let responseMessage = `Erreur: Echec lors lors de la synchro des données dans DHIS2`;
    ...
    getListHapiResourceByFilter(hapiToken, fhirProgramResource, filterExpresion, (listProgramsEntries) => {
      ...
      async.eachSeries(listProgramsEntries, function (programEntry, nextStep) {
        ...
        logger.log({
          level: levelType.info, operationType: typeOperation.getData, action: "/updatecatcombodhis", result: typeResult.iniate,
          ...
          if (resourceDhisCatcombos.length > 0) {
            ...
            getListDHISResourceByParamsChunkWithNoIdentified(dhis2Token, dhisCategoryComboOptions, listProductName, 20,
              (listDhisCategoryComboOptions) => {
                ...
                let listCatComboPayLoadToUpdate = customLibrairy.buildCategoryComboOptionsMetadata(listProductDetails,
                  listDhisCategoryComboOptions[0], catComboId);
                //return res.send([listCatComboPayLoadToUpdate,listCatComboPayLoadToCreate]);

                logger.log({
                  level: levelType.info, operationType: typeOperation.getData, action: "/updatecatcombodhis", result: typeResult.success,
                ...
                logger.log({
                  level: levelType.info, operationType: typeOperation.postData, action: "/updatecatcombodhis", result: typeResult.iniate,
                ...
                saveMetadataList2Dhis(dhis2Token, dhisCategoryComboOptions, listCatComboPayLoadToCreate,
                  (dhisCreateOperation) => {
                    ...
                    if (httpStatusCreated && httpStatusCreated.length > 0) {
                      logger.log({
                        level: levelType.info, operationType: typeOperation.postData, action: "/updatecatcombodhis", result: typeResult.success,
                    ...
                    updateMetadataList2Dhis(dhis2Token, dhisCategoryComboOptions, listCatComboPayLoadToUpdate,
                      (dhisUpdateOperation) => {
...
function saveEntryToElastic(url, esToken, logEntries, callbackMain) {
  ...
  async.each(logEntries, function (logEntry, callback) {
    ...
    if (err) {
      //console.log(err);
      logger.log({
        level: levelType.error, operationType: typeOperation.getData, action: `/${url}`, result: typeResult.failed,
        message: `${err.Error}`
      });
    }
...
function saveBundle2Fhir(fhirToken, fhirResource, bundle, callback) {
  ...
  localNeedle.post(url, JSON.stringify(bundle), options, function (err, resp) {
    if (err) {
      logger.log({
        level: levelType.error, operationType: typeOperation.postData, action: `/${url}`, result: typeResult.failed,
        message: `${err}`
      });
...
function validateFHIREntryBundle(fhirToken, fhirResource, bundle, resourceProfile, callback) {
  let localNeedle = require('needle');
  localNeedle.defaults(
    {
      open_timeout: 600000
...
function saveMetadataList2Dhis(dhis2Token, dhisResource, listMetadata, callback) {
  ...
  localAsync.eachSeries(listMetadata, function (metadata, nextResource) {
    ...
    if (listAlreadyExistedResources.length > 0) {
      ...
      logger.log({
        level: levelType.warning, operationType: typeOperation.getData, action: `/api/${dhisResource}`, result: typeResult.failed,
...
function saveMetadataCollection2Dhis(dhis2Token, dhisParentResource, parentId, dhisCollectionResource,
  ...
  localNeedle.post(url, JSON.stringify(metaDataCollection), options, function (err, resp) {
    ...
    if (httpStatus != "OK") {
      logger.log({
        level: levelType.warning, operationType: typeOperation.postData, action: `/${url}`, result: typeResult.failed,
...
function updateMetadataList2Dhis(dhis2Token, dhisResource, listMetadata, callback) {
  ...
  localAsync.eachSeries(listMetadata, function (metadata, nextResource) {
    ...
    localNeedle.put(url, JSON.stringify(metadata), options, function (err, resp) {
      if (err) {
        logger.log({
          level: levelType.error, operationType: typeOperation.postData, action: `/${dhisResource}`, result: typeResult.failed,
          message: `${err.Error}`
        });
        nextResource(err);
...
function start(callback) {
  ...
  if (apiConf.register) {
    medUtils.registerMediator(apiConf.api, mediatorConfig, (err) => {
      if (err) {
        logger.log({
          level: levelType.error, operationType: typeOperation.normalProcess, action: `Enregistrement du mediateur`, result: typeResult.failed,
      ...
      medUtils.fetchConfig(apiConf.api, (err, newConfig) => {
        logger.log({
          level: levelType.info, operationType: typeOperation.normalProcess, action: "Reception des configurations intiales",
        ...
        if (err) {
          logger.log({
            level: levelType.error, operationType: typeOperation.normalProcess, action: `Echec d'obtention des configurations initiales`, result: typeResult.failed,
          ...
          const server = app.listen(port, () => {
            if (apiConf.heartbeat) {
              ...
              configEmitter.on('config', (newConfig) => {
                logger.log({
                  level: levelType.info, operationType: typeOperation.normalProcess, action: "Obtension des configuratiobns a jour",
...
process.on('uncaughtException', err => {
  console.log(err);
  logger.log({
    level: levelType.error, operationType: typeOperation.stopTheService, action: `arret anormal du mediateur sur l'action `, result: typeResult.failed,
...
process.on('SIGTERM', signal => {
  logger.log({
    level: levelType.info, operationType: typeOperation.stopTheService, action: "Arret du mediateur", result: typeResult.success,
...
process.on('SIGINT', signal => {
  logger.log({
    level: levelType.error, operationType: typeOperation.stopTheService, action: "Arret brusque du mediateur", result: typeResult.failed,
