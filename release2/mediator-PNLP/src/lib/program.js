...
function setupApp() {
  ...
  */
  ...
  //Generate the metadata (DataElement) in DHIS2 as defined in config/dhismetadatadef.json. this will content the requisition data of the product
  app.get('/genprogmetadata', (req, res) => {
    globalRes = res;
    var operationOutcome = true;
    logger.log({
      level: levelType.info, operationType: typeOperation.normalProcess, action: "/genprogmetadata", result: typeResult.iniate,
    ...
    logger.log({
      level: levelType.info, operationType: typeOperation.getData, action: `/api/${dhisCategoryCombo}`, result: typeResult.iniate,
    ...
    getListDHIS2ResourceByFilter(dhis2Token, dhisCategoryCombo, filterExpression, (resCategoryCombination) => {
      if (resCategoryCombination && resCategoryCombination.length > 0) {
        logger.log({
          level: levelType.info, operationType: typeOperation.getData, action: `/api/${dhisCategoryCombo}`, result: typeResult.success,
          message: `DHIS2: Extraction de l'ID de ${config.program.code}`
        });
        ...
        logger.log({
          level: levelType.info, operationType: typeOperation.getData, action: `/api/dataElements`, result: typeResult.iniate,
        ...
        saveMetadataList2Dhis(dhis2Token, dhisDataElement, listDataElementsMetadata, (resOpCreationDataElement) => {
          ...
          if (resOpCreationDataElement && resOpCreationDataElement.length > 0) {
            ...
            logger.log({
              level: levelType.info, operationType: typeOperation.getData, action: `/api/dataElements`, result: typeResult.success,
            ...
            updateMetadataList2Dhis(dhis2Token, dhisDataElement, listDataElements2Update, (dhisUpdateOperation) => {
              logger.log({
                level: levelType.info, operationType: typeOperation.postData, action: `/api/dataElements`, result: typeResult.success,
            ...
            logger.log({
              level: levelType.info, operationType: typeOperation.getData, action: `/api/dataElements`, result: typeResult.failed,
              message: `DHIS2: Erreur lors de la creation des elements des donnees du programme`
            });
            logger.log({
              level: levelType.info, operationType: typeOperation.getData, action: `/genprogmetadata`, result: typeResult.failed,
              message: `DHIS2: Erreur lors de la creation des elements des donnees du programme`
            });
            let urn = mediatorConfig.urn;
            let status = '';
        ...
        logger.log({
          level: levelType.info, operationType: typeOperation.getData, action: `/api/${dhisCategoryCombo}`, result: typeResult.failed,
  ...
  app.get('/_syncrequisition2fhir', (req, res) => {
    ...
    var operationOutcome = true;
    logger.log({
      level: levelType.info, operationType: typeOperation.normalProcess, action: "/syncrequisition2fhir", result: typeResult.iniate,
      ...
      if (locationEntries.length > 0) {
        ...
        customLibrairy.readeSIGLRequisitionCSVFile(filePath, (listAllRequisitions) => {
          ...
          if (listAllRequisitions && listAllRequisitions.length > 0) {
            ...
            logger.log({
              level: levelType.info, operationType: typeOperation.getData, action: `/${filePath}`, result: typeResult.success,
            ...
            logger.log({
              level: levelType.info, operationType: typeOperation.getData, action: `/fhir/${fhirLocationResource}`, result: typeResult.iniate,
            ...
            async.eachSeries(listProgramRequisitions, function (reqRecord, nextRecord) {
              ...
              if (err) {
                logger.log({
                  level: levelType.error, operationType: typeOperation.getData, action: `/fhir/${fhirLocationResource}`,
                  result: typeResult.failed, message: `Extraction des details sur les structures pour le traitement des requisitions. ${err}`
                });
                operationOutcome = operationOutcome && false;
              }
              logger.log({
                level: levelType.info, operationType: typeOperation.getData, action: `/fhir/${fhirLocationResource}`, result: typeResult.success,
              ...
              if (facilitiesListNotFound.length > 0) {
                ...
                logger.log({
                  level: levelType.warning, operationType: typeOperation.getData, action: `/fhir/${fhirLocationResource}`, result: typeResult.failed,
              ...
              logger.log({
                level: levelType.info, operationType: typeOperation.postData, action: `/fhir/requisition`, result: typeResult.iniate,
              ...
              saveBundle2Fhir(hapiToken, fhirRequisitionResource, bundleRequisition, (hapiRequisitionBundleResponse) => {
                if (hapiRequisitionBundleResponse.status == 200) {
                  operationOutcome = operationOutcome && true;
                  logger.log({
                    level: levelType.info, operationType: typeOperation.postData, action: `/fhir/requisition`, result: typeResult.success,
                    message: `HAPI: ${bundleRequisition.entry.length}  Insertion des requisitions dans HAPI`
                  });
                  logger.log({
                    level: levelType.info, operationType: typeOperation.postData, action: `/syncrequisition2fhir`, result: typeResult.success,
                  ...
                  operationOutcome = operationOutcome && false;
                  logger.log({
                    level: levelType.info, operationType: typeOperation.postData, action: `/fhir/requisition`, result: typeResult.failed,
                    message: `${hapiRequisitionBundleResponse.message}: Echec de la mise a jour des Requisitions`
                  });
                  logger.log({
                    level: levelType.info, operationType: typeOperation.postData, action: `/syncrequisition2fhir`, result: typeResult.failed,
                    message: `${hapiRequisitionBundleResponse.message}: Echec de la mise a jour des Requisitions`
                  });
                }
            ...
            logger.log({
              level: levelType.error, operationType: typeOperation.getData, action: `/syncrequisition2fhir`, result: typeResult.failed,
              message: `Erreur lors la lecture du fichier CSV de requisitions`
            });
        ...
        logger.log({
          level: levelType.error, operationType: typeOperation.getData, action: `/fhir/${fhirLocationResource}`, result: typeResult.failed,
  ...
  app.get('/syncrequisition2fhir', (req, res) => {
    ...
    getListHapiResourceFilteredByParams(hapiToken, fhirResource, keyValueParmsList, function (listPrefectures) {
      logger.log({
        level: levelType.info, operationType: typeOperation.normalProcess, action: "/syncrequisition2fhir", result: typeResult.success,
                    ...
                    getListHapiResourceFilteredByParamsFromIteration(hapiToken, fhirResource, listkeyValueParmsList,
                      function (listePosteSante) {
                        ...
                        if (listePosteSante && listePosteSante.length > 0) {
                          ...
                          logger.log({
                            level: levelType.warning, operationType: typeOperation.normalProcess, action: "/syncrequisition2fhir", result: typeResult.success,
  ...
  app.get('/generatedaeValues', (req, res) => {
    ...
    if (syncPeriod.split("-").length != 2) {
      logger.log({
        level: levelType.info, operationType: typeOperation.normalProcess, action: "/syncrequisition2dhis", result: typeResult.iniate,
    ...
    logger.log({
      level: levelType.info, operationType: typeOperation.getData, action: `/fhir/requisition`, result: typeResult.iniate,
    ...
    getListHapiResourceByFilterCurl(hapiToken, fhirRequisitionResource, filterExpresion, (requisitionEntries) => {
      ...
      if (requisitionEntries.length > 0) {
        logger.log({
          level: levelType.info, operationType: typeOperation.getData, action: `/fhir/requisition`, result: typeResult.success,
        ...
        logger.log({
          level: levelType.info, operationType: typeOperation.normalProcess, action: `/syncrequisition2dhis`, result: typeResult.ongoing,
        ...
        //return res.send("OK");
        logger.log({
          level: levelType.info, operationType: typeOperation.getData, action: `/fhir/programme`, result: typeResult.iniate,
        ...
        getListHapiResourceByFilter(hapiToken, fhirProgramResource, filterProgram, (programResource) => {
          ...
          if (programResource.length > 0) {
            ...
            logger.log({
              level: levelType.info, operationType: typeOperation.getData, action: `/fhir/programme`, result: typeResult.success,
            ...
            getListHapiResourceByFilter(hapiToken, fhirProductResource, filterProduct, (productsResource) => {
              if (productsResource.length > 0) {
                ...
                logger.log({
                  level: levelType.info, operationType: typeOperation.normalProcess, action: "/syncrequisition2fhir", result: typeResult.success,
                  message: `Generation indicateurs: ${newListDataElement2Push.length}`
                });
                ...
                saveAdxData2Dhis(dhis2Token, adxDataElementObjectLists, (adxSaveResults) => {
                  if (adxSaveResults) {
                    ...
                    else if (config.dhis2ResponseVersion == "2.41") {
                    ...
                    if (importChildStatus == "SUCCESS") {
                      logger.log({
                        level: levelType.info, operationType: typeOperation.postData, action: `/syncrequisition2dhis`, result: typeResult.success,
                      ...
                      logger.log({
                        level: levelType.info, operationType: typeOperation.postData, action: `/api/saveAdxData2Dhis`, result: typeResult.failed,
                        message: `Echec de l'importation des donnees. Code d'erreur ${importChildStatus}`
                      });
                      logger.log({
                        level: levelType.info, operationType: typeOperation.postData, action: `/syncrequisition2dhis`, result: typeResult.failed,
                        message: `Echec de l'importation des donnees. Code d'erreur ${importChildStatus}`
                      });
                    ...
                    logger.log({
                      level: levelType.error, operationType: typeOperation.postData, action: `/api/saveAdxData2Dhis`, result: typeResult.failed,
                      message: `Aucune reponse du server DHIS2 lors de l'envoi de donnees des requisitions`
                    });
                    logger.log({
                      level: levelType.error, operationType: typeOperation.postData, action: `/syncrequisition2dhis`, result: typeResult.failed,
                      message: `Aucune reponse du server DHIS2 lors de l'envoi de donnees des requisitions`
                    });
                ...
                logger.log({
                  level: levelType.error, operationType: typeOperation.getData, action: `/fhir/product`, result: typeResult.failed,
                  message: `Erreur: Echec lors de l'extraction des produits [${stringProductIdsList}] par programme : ${config.program.code} `
                });
                logger.log({
                  level: levelType.error, operationType: typeOperation.getData, action: `/syncrequisition2dhis`, result: typeResult.failed,
                  message: `Erreur: Echec lors de l'extraction des produits [${stringProductIdsList}] par programme : ${config.program.code} `
                });
            ...
            logger.log({
              level: levelType.error, operationType: typeOperation.getData, action: `/fhir/programme`, result: typeResult.failed,
        ...
        logger.log({
          level: levelType.error, operationType: typeOperation.getData, action: `/fhir/requisition`, result: typeResult.failed,
  ...
  localAsync.whilst(
    ...
    callback => {
      ...
      exec('curl ' + args, function (error, stdout, stderr) {
        ...
        if (error !== null) {
          logger.log({
            level: levelType.error, operationType: typeOperation.getData, action: `/${fhirResource}`, result: typeResult.failed,
            message: `${error}`
          });
          return callback(true, false);
        }
  ...
  localAsync.whilst(
    ...
    callback => {
      ...
      localNeedle.get(url, options, function (err, resp) {
        ...
        if (body.entry && body.entry.length > 0) {
          logger.log({
            level: levelType.info, operationType: typeOperation.getData, action: `/${fhirResource} page:${body.entry.length}/${body.total}`,
            result: typeResult.success, message: `Extraction de  ${body.entry.length} ${fhirResource} de HAPI`
          });
          resourceData = resourceData.concat(body.entry);
          //force return only one loop data
...
function getListApprovedRequisitionsByFacility3(eSIGLToken, keyValueParmsList, dhis2FacilityId, callbackMain) {
  ...
  localAsync.whilst(
    ...
    callback => {
      ...
      localNeedle.get(url, options, function (err, resp) {
        ...
        if (body.data && body.data.rows.length > 0) {
          ...
          logger.log({
            level: levelType.info, operationType: typeOperation.getData, action: `/${initUrl}`, result: typeResult.success,
    ...
    err => {
      ...
      logger.log({
        level: levelType.info, operationType: typeOperation.getData, action: `/getListApprovedRequisitionsByFacility3`, result: typeResult.success,
...
function saveAdxData2Dhis(dhis2Token, adxPayload, callback) {
  ...
  localNeedle.post(url, adxPayload, options, function (err, resp) {
    if (err) {
      logger.log({
        level: levelType.error, operationType: typeOperation.postData, action: `/dataValueSets`, result: typeResult.failed,
        message: `${err.Error}`
      });
      callback({});
    }
...
process.on('SIGINT', signal => {
  logger.log({
    level: levelType.error, operationType: typeOperation.stopTheService, action: "Arret brusque du mediateur", result: typeResult.success,
